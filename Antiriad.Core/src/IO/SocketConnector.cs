...
public class SocketConnector : Connector
{
  ...
  private void DoClose(Socket? client)
  {
    ...
    if (this.socketType == SocketType.Stream)
    {
      try { client.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.Linger, false); }
      catch (Exception) { }
    }
  ...
  protected override int DeviceRead(byte[] buffer, int offset, int count, int timeout)
  {
    ...
    try { received = readable ? client!.Receive(buffer, offset, count, SocketFlags.None) : -1; }
    catch (Exception) { }
  ...
  {
    try { return this.socket?.ReceiveBufferSize ?? 0; }
    catch (Exception) { return 0; }
  }

  protected override int GetSendBufferSize()
  {
    try { return this.socket?.SendBufferSize ?? 0; }
    catch (Exception) { return 0; }
  }
}
